...
async function generateSummaryForProduct(
  ...
  const { data: product } = await supabase
    ...
    .select(`
      ...
      specs_common ( cpu_name, gpu_name, ram_gb, storage_gb, storage_type, os ),
  ...
  const specLines = [
    ...
    (s ?? t)?.battery_mah ? `Battery: ${(s ?? t).battery_mah}mAh`                                   : null,
    s?.camera_main_mp ? `Camera: ${s.camera_main_mp}MP main / ${s.camera_front_mp ?? '?'}MP front`  : null,
    l?.weight_kg      ? `Weight: ${l.weight_kg}kg`                                                   : null,
    c?.os             ? `OS: ${c.os}`                                                                : null,
  ].filter(Boolean).join('\n')

  const prompt = `You are a tech product editor for a comparison website. Write a 2–3 sentence editorial summary for the product below.

Rules:
- Highlight key strengths and who this product is best for
- Do NOT mention price, cost, or value for money
- Do NOT include negative points, trade-offs, or criticisms
- Plain text only, no markdown, no bullet points
- Return a JSON object with keys: en, es, pt, fr, de, ja, ko — each containing the summary in that language
...
Respond with only the JSON object, nothing else.`
  ...
  try {
    i18n = JSON.parse(jsonStr)
  } catch {
    return { ok: false, error: `JSON parse failed: ${raw.slice(0, 100)}` }
  }

  const { error } = await supabase
    .from('products')
    .update({ ai_summary_i18n: i18n })
    .eq('id', productId)
...
// body: { batch: true }        — ai_summary_i18n 없는 전체 제품 5개씩 처리
